...
import { ChartData, Transaction, UserDetails } from '@/hooks/dashboard/super-admins/rekap/lib/rekap';
...
export default function RekapLayout() {
        ...
        try {
            ...
            const unsubscribe = onSnapshot(q, (querySnapshot) => {
                ...
                const userDailyData = transactionData.reduce<Record<string, Record<string, number>>>((acc, transaction) => {
                    ...
                    const userName = transaction.userDetails.fullName;
    ...
    const userSummary = transactions.reduce<Record<string, {
        ...
        const userId = t.userDetails.userId || t.userDetails.fullName;

        if (!acc[userId]) {
            acc[userId] = {
                count: 0,
                total: 0,
                userInfo: t.userDetails,
        ...
        const productId = t.productDetails.id;
        if (!acc[userId].products[productId]) {
            acc[userId].products[productId] = {
                count: 0,
                title: t.productDetails.title
            };
        }
        acc[userId].products[productId].count += 1;
    ...
    const getSortedAndFilteredUsers = () => {
        ...
        if (searchQuery.trim() !== '') {
            ...
            filteredUsers = filteredUsers.filter(([, data]) => {
                const fullName = data.userInfo?.fullName?.toLowerCase() || '';
                const email = data.userInfo?.email?.toLowerCase() || '';
                                ...
                                >
                                    <div className="flex items-start gap-4 mb-4">
                                        <div className="relative">
                                            <Image
                                                src={data.userInfo?.photoURL || '/default-avatar.png'}
                                                alt={data.userInfo?.fullName || 'User'}
                                                width={48}
                                                height={48}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h4 className="font-semibold text-gray-800 truncate">
                                                {data.userInfo?.fullName || 'Unknown User'}
                                            </h4>
                                            <p className="text-sm text-gray-500 truncate">
                                                {data.userInfo?.email || 'No email'}
                                            </p>
