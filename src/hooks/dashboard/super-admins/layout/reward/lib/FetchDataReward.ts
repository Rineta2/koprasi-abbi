...
export const useRewardData = () => {
  ...
  const [rewards, setRewards] = useState<RewardsContent[]>([]);
  ...
  const fetchContents = async () => {
    try {
      const querySnapshot = await getDocs(
        collection(
          db,
          process.env.NEXT_PUBLIC_COLLECTIONS_REWARDS as string
        )
      );
  ...
  const createContent = async (data: RewardsContent) => {
    await addDoc(
      collection(
        db,
        process.env.NEXT_PUBLIC_COLLECTIONS_REWARDS as string
  ...
  const handleUpdate = async (
    ...
    try {
      ...
      await updateDoc(docRef, {
        ...
        textLeft: updatedData.textLeft,
        textRight: updatedData.textRight,
        syarat: updatedData.syarat,
        updatedAt: new Date(),
      });
      await fetchContents();
    } catch (error) {
