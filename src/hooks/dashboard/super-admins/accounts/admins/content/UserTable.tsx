...
export default function UserTable({ users, onEdit, onDelete, deletingId }: UserTableProps) {
    return (
        <div className="rounded-3xl shadow-sm border border-gray-200 overflow-hidden bg-card">
            <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                <table className="w-full min-w-[800px]">
                    <thead>
                        <tr className="border-b border-gray-200 bg-background-dark">
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {users.map((user) => (
                            <tr key={user.uid} className="hover:bg-card-hover transition-colors duration-200">
                                <td className="px-6 py-4 text-sm font-medium text-text">{user.fullName}</td>
                                <td className="px-6 py-4 text-sm text-text-dark">{user.email}</td>
